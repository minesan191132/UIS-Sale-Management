                ...
                <div class="col-md-6 text-md-end mt-3 mt-md-0">
                  ...
                  <div v-if="order.totalPrice" class="mb-3">
                    ...
                    <p class="mb-0">
                      ...
                      <strong class="text-success">{{ formatCurrency(Math.round(order.totalPrice * 0.6)) }}</strong>
                    </p>
                  </div>
          ...
          <div class="modal-body">
            ...
            <div class="row mb-3 pb-3 border-bottom">
              ...
              <div class="col-md-4">
                ...
                <p class="mb-1" v-if="selectedOrder.totalPrice">
                  <strong>Cọc trước 60%:</strong>
                  <span class="text-success fw-bold">{{ formatCurrency(Math.round(selectedOrder.totalPrice * 0.6)) }}</span>
                </p>
              </div>
              <div class="col-md-4">
...
const showPaymentQR = (order) => {
  // Tính toán số tiền cọc 60%
  const depositAmount = Math.round(order.totalPrice * 0.6)
  
  // Tạo URL QR Code theo chuẩn SePay
  // 1. Khai báo thông tin tài khoản đã liên kết với SePay
  const bank = 'MBBank' // Tên viết tắt ngân hàng (VD: MBBank, Vietcombank, ACB...)
  const acc = '0934103182' // Số tài khoản của bạn
  
  // 2. Số tiền và nội dung
  const amount = depositAmount // Số tiền cọc 60%
  const des = order.orderNumber // Mã đơn hàng (bắt buộc)
  
  // 3. Tạo link ảnh QR của SePay
  ...
  Swal.fire({
    ...
    html: `
      <div class="text-center">
        <p class="mb-3">Quét mã QR để thanh toán cọc <strong class="text-success">${formatCurrency(depositAmount)}</strong></p>
